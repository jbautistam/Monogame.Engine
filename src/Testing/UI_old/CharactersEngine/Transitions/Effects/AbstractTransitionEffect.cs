...
public abstract class AbstractTransitionEffect(TransitionManager manager, string shaderFile)
{
        ...
        if (!IsComplete)
        {
            float rawProgress;
                ...
                if (Easing is not null)
                    Progress = MathTools.Easing.EasingFunctionsHelper.Apply(rawProgress, Easing ?? MathTools.Easing.EasingFunctionsHelper.EasingType.Linear);
                else
    ...
    /// </summary>
    public MathTools.Easing.EasingFunctionsHelper.EasingType? Easing { get; }

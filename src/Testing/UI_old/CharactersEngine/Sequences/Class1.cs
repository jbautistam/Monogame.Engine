...
// Entrada dramática cinematográfica
public class CinematicEntranceCommand : AbstractSequenceCommand
{
    public enum EntranceType { Slam, Slide, Drop, Leap, Teleport, Emerging, Descending }
    
    private readonly EntranceType _type;
    private readonly Vector2 _from;
    private readonly Vector2 _to;
    private readonly float _impactPause;
    private readonly float _anticipation;
    private Vector2 _startPos;
    private bool _impacted;
    private bool _initialized;
    
    public CinematicEntranceCommand(string actorId, float startTime, float duration,
        EntranceType type, Vector2 from, Vector2 to, float impactPause = 0.1f, float anticipation = 0.2f)
    ...
    public override void Apply(Actor actor, float sequenceTime)
    {
        ...
        {
            actor.Transform.Position = _from;
            return;
        }
        ...
        switch (_type)
        {
            ...
            case EntranceType.Teleport:
                ApplyTeleport(actor, t);
                break;
            case EntranceType.Emerging:
                ApplyEmerging(actor, t);
                break;
            case EntranceType.Descending:
                ApplyDescending(actor, t);
                break;
        }
    }
    
    private void ApplySlam(Actor actor, float t)
    {
        // Anticipación (retrocede ligeramente) -> movimiento rápido -> impacto -> pausa
        float anticipationEnd = _anticipation;
        float moveEnd = 1 - _impactPause / Duration;
        
        if (t < anticipationEnd)
        {
            float localT = t / anticipationEnd;
            actor.Transform.Position = Vector2.Lerp(_from, _from - new Vector2(50, 0), 
                Easing.BackIn(localT));
        }
        else if (t < moveEnd)
        {
            float localT = (t - anticipationEnd) / (moveEnd - anticipationEnd);
            actor.Transform.Position = Vector2.Lerp(_from - new Vector2(50, 0), _to, 
                Easing.ExpoOut(localT));
        }
        else
        {
            if (!_impacted)
            {
                _impacted = true;
                // Aquí se dispararía efecto de impacto
            }
            actor.Transform.Position = _to;
        }
    }
    
    private void ApplySlide(Actor actor, float t)
    {
        // Entra rápido, frena con fricción
        if (t < 0.7f)
        {
            float localT = t / 0.7f;
            actor.Transform.Position = Vector2.Lerp(_from, _to, Easing.QuadOut(localT));
        }
        else
        {
            float localT = (t - 0.7f) / 0.3f;
            // Overshoot pequeño y vuelta
            float overshoot = MathF.Sin(localT * MathF.PI) * 20 * (1 - localT);
            actor.Transform.Position = _to + new Vector2(overshoot, 0);
        }
    }
    
    private void ApplyDrop(Actor actor, float t)
    {
        // Caída acelerada por gravedad
        float yOffset = Easing.QuadIn(t) * (_to.Y - _from.Y);
        float xOffset = Easing.Linear(t) * (_to.X - _from.X);
        actor.Transform.Position = new Vector2(_from.X + xOffset, _from.Y + yOffset);
        
        // Escala al impactar (achatamiento)
        if (t > 0.9f)
        {
            float squash = 1 - (t - 0.9f) * 10 * 0.2f;
            actor.Transform.Scale = new Vector2(1.1f, squash);
        }
    }
    
    private void ApplyLeap(Actor actor, float t)
    {
        // Parábola alta
        Vector2 midPoint = (_from + _to) / 2 + new Vector2(0, -200);
        
        if (t < 0.5f)
        {
            float localT = t * 2;
            actor.Transform.Position = Vector2.Lerp(
                Vector2.Lerp(_from, midPoint, localT),
                Vector2.Lerp(midPoint, _to, localT),
                Easing.QuadOut(localT));
        }
        else
        {
            ...
            actor.Transform.Position = Vector2.Lerp(
                Vector2.Lerp(midPoint, _to, localT),
                _to,
                Easing.QuadIn(localT));
        }
    }
    
    private void ApplyTeleport(Actor actor, float t)
    {
        // Desaparece en origen, aparece en destino con efecto
        float fadeOutEnd = 0.2f;
        float fadeInStart = 0.4f;
        
        if (t < fadeOutEnd)
        {
            actor.Transform.Opacity = 1 - t / fadeOutEnd;
            actor.Transform.Position = _from;
        }
        else if (t < fadeInStart)
        {
            actor.Transform.Opacity = 0;
        }
        else
        {
            float localT = (t - fadeInStart) / (1 - fadeInStart);
            actor.Transform.Opacity = localT;
            actor.Transform.Position = _to;
            // Escala de "materialización"
            actor.Transform.Scale = 1 + MathF.Sin(localT * MathF.PI) * 0.3f;
        }
    }
    
    private void ApplyEmerging(Actor actor, float t)
    {
        // Sube desde abajo con opacidad creciente
        Vector2 emergeFrom = new Vector2(_to.X, _to.Y + 100);
        
        actor.Transform.Position = Vector2.Lerp(emergeFrom, _to, Easing.BackOut(t));
        actor.Transform.Opacity = MathHelper.Clamp(t * 2, 0, 1);
    }
    
    private void ApplyDescending(Actor actor, float t)
    {
        // Baja desde arriba con oscilación de cuerda
        Vector2 descendFrom = new Vector2(_to.X + 50, _to.Y - 300);
        
        float sway = MathF.Sin(t * MathF.PI * 3) * 30 * (1 - t);
        Vector2 basePos = Vector2.Lerp(descendFrom, _to, Easing.QuadIn(t));
        actor.Transform.Position = basePos + new Vector2(sway, 0);
...
// Pose dramática con estilo
public class DramaticPoseCommand : AbstractSequenceCommand
{
    public enum PoseStyle { Heroic, Villainous, Comedic, Tragic, Mysterious }
    
    private readonly Vector2 _position;
    private readonly float _scale;
    private readonly float _rotation;
    private readonly PoseStyle _style;
    private Vector2 _startPos;
    private float _startScale;
    private float _startRotation;
    private bool _initialized;
    
    public DramaticPoseCommand(string actorId, float startTime, float duration,
        Vector2 position, float scale, float rotationDegrees, PoseStyle style)
        : base(actorId, startTime, duration)
    {
        _position = position;
        _scale = scale;
    ...
    public override void Apply(Actor actor, float sequenceTime)
    {
        ...
        {
            _startPos = actor.Transform.Position;
            _startScale = actor.Transform.Scale;
            _startRotation = actor.Transform.Rotation;
            _initialized = true;
        }
        ...
        float easedT = ApplyStyleEasing(t);
        
        actor.Transform.Position = Vector2.Lerp(_startPos, _position, easedT);
        actor.Transform.Scale = MathHelper.Lerp(_startScale, _scale, easedT);
        actor.Transform.Rotation = MathHelper.Lerp(_startRotation, _rotation, easedT);
        
        // Efectos adicionales por estilo
        ApplyStyleEffects(actor, t);
    }
    
    private float ApplyStyleEasing(float t)
    {
        return _style switch
        {
            PoseStyle.Heroic => Easing.BackOut(t),
            PoseStyle.Villainous => Easing.CubicIn(t),
            PoseStyle.Comedic => Easing.ElasticOut(t),
            PoseStyle.Tragic => Easing.QuadInOut(t),
            PoseStyle.Mysterious => Easing.SmoothStep(t),
            _ => t
        };
    }
    
    private void ApplyStyleEffects(Actor actor, float t)
    {
        switch (_style)
        {
            case PoseStyle.Heroic:
                // Brillo sutil
                actor.Transform.Color = Color.Lerp(Color.White, Color.LightYellow, t * 0.3f);
                break;
            case PoseStyle.Villainous:
                // Oscurecimiento
                actor.Transform.Color = Color.Lerp(Color.White, Color.DarkGray, t * 0.4f);
                break;
            case PoseStyle.Tragic:
                // Desaturación
                float gray = 0.8f - t * 0.3f;
                actor.Transform.Color = new Color(gray, gray, gray);
                break;
            case PoseStyle.Mysterious:
                // Transparencia fluctuante
                actor.Transform.Opacity = 0.8f + MathF.Sin(t * MathF.PI * 4) * 0.2f;
                break;
        }
    }
}
...
public class OrbitCommand : AbstractSequenceCommand
{
    ...
    public OrbitCommand(string actorId, float startTime, float duration,
        Vector2 center, float radius, float startAngleDegrees, float endAngleDegrees,
        bool clockwise = true, IEasingDefinition easing = null)
        : this(actorId, startTime, duration, center, radius, radius, startAngleDegrees, endAngleDegrees, clockwise, easing) { }
    
    public OrbitCommand(string actorId, float startTime, float duration,
        Vector2 center, float radiusX, float radiusY, float startAngleDegrees, float endAngleDegrees,
        bool clockwise = true, IEasingDefinition easing = null)
        : base(actorId, startTime, duration)
    {
        _center = center;
        _radiusX = radiusX;
        _radiusY = radiusY;
        _startAngle = MathHelper.ToRadians(startAngleDegrees);
        _endAngle = MathHelper.ToRadians(endAngleDegrees);
    ...
    public override void Apply(Actor actor, float sequenceTime)
    {
        ...
        if (sequenceTime >= EndTime)
        {
            float finalAngle = _clockwise ? _endAngle : -_endAngle;
            actor.Transform.Position = CalculatePosition(finalAngle);
            return;
        }
        ...
        float angleDiff = _endAngle - _startAngle;
        if (!_clockwise) angleDiff = -angleDiff;
        
        float currentAngle = _startAngle + angleDiff * easedT;
        actor.Transform.Position = CalculatePosition(currentAngle);
        
        // Rotar sprite para mirar hacia la dirección de movimiento
    ...
    {
        return new Vector2(
            _center.X + MathF.Cos(angle) * _radiusX,
            _center.Y + MathF.Sin(angle) * _radiusY
        );
    }
}

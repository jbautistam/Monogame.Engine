...
using UI.CharactersEngine.ParticlesEngine;
using UI.CharactersEngine.ParticlesEngine.Modifiers;
...
public class ParticleSystemDefinitionCollection
{
	...
	/// </summary>
	public CharactersEngine.ParticlesEngine.ParticleEngine? Create(string name, Vector2 position)
	{
		if (Definitions.TryGetValue(name, out ParticleSystemDefinitionModel? system))
			return Create(system, position);
		else
			return null;
	}
	...
	///		Crea un sistema de partículas
	/// </summary>
	public ParticleEngine? Create(ParticleSystemDefinitionModel system, Vector2 position)
	{
		ParticleEngine engine = new();
	...
	private AbstractParticleModifier CreateOpacityModifier(ParticleSystemModifierDefinitionModel modifier)
	{
		return new AlphaOverLifetimeModifier(GetInt("Minimum", modifier.Parameters, 0),
											 ...
											 CharactersEngine.MathTools.Easing.EasingFunctionsHelper.EasingType.Linear);
	}

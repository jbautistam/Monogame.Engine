...
internal class CommandTypeWriterGenerator
{
	...
	/// </summary>
	internal List<CommandAbstractLine> Parse(string text)
	{
		List<StringExtractor.Token> tokens = new StringExtractor().Extract(text, '[', ']');
		List<CommandAbstractLine> commands = [];
			...
			foreach (StringExtractor.Token token in tokens)
				if (token.IsCode)
				{
					TokenExtended extended = ParseToken(token.Text);
						...
						switch (extended.Type)
						{
							case TokenType.Color:
								...
								break;
							case TokenType.Wait:
									commands.Add(new CommandWaitTypeWriter()
															{
							...
							case TokenType.Speed:
									commands.Add(new CommandSpeedTypeWriter()
															{
							...
							case TokenType.Event:
									commands.Add(new CommandEvetTypeWriter()
															{
																Data = extended.Text
															}
												);
					...
					commands.Add(CreateTextCommand(token.Text));
	...
	{
		if (!string.IsNullOrWhiteSpace(text) && float.TryParse(text, out float value))
			return value;
		else
			return defaultValue;
	}
	...
	/// </summary>
	private CommandTextTypeWriter CreateTextCommand(string text)
	{
		return new CommandTextTypeWriter()
							{
								Text = text,
								Color = GetLastColor(),
								Bold = _bold,

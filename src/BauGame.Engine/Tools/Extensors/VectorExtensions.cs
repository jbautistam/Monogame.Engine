...
public static class VectorExtensions
{
    ...
    /// </summary>
    public static Vector2 RotateAround(this Vector2 source, Vector2 origin, float radians)
    {
        Vector2 diff = source - origin;
    ...
    {
        return new Vector2(MathHelper.Lerp(start.X, end.X, MathHelper.Clamp(t, 0, 1)), MathHelper.Lerp(start.Y, end.Y, MathHelper.Clamp(t, 0, 1)));
    }
    ...
    ///     Signo del vector
    /// </summary>
    public static Vector2 Sign(this Vector2 vector) => new(Math.Sign(vector.X), Math.Sign(vector.Y));
